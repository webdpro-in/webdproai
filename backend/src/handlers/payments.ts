...
export const setupMerchantPayments = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const razorpayAccount = await partnerRazorpay.accounts.create({
      ...
      profile: {
        ...
        addresses: {
          registered: {
            street1: 'Auto-generated address',
            city: 'Mumbai',
            state: 'Maharashtra',
            postal_code: '400001',
      },
...
export const createOrderPayment = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    const razorpayOrder = await partnerRazorpay.orders.create({
      ...
      'X-Razorpay-Account': merchant.Item.razorpay_account_id // Route to merchant's account
    });
