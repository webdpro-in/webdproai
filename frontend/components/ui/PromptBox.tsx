...
import { cn } from "@/lib/utils" // Ensure utils is available or use alias
...
export function PromptBox({ onSubmit, isGenerating }: PromptBoxProps) {
   ...
   return (
      <div className="w-full max-w-3xl mx-auto relative group z-20">
         ...
         <form onSubmit={handleSubmit} className="relative">
            ...
            >
               ...
               <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 text-indigo-400 ml-2">
                  <Sparkles className={cn("w-6 h-6", isGenerating ? "animate-spin" : "animate-pulse")} />
               </div>
         ...
         <div className={cn("absolute top-full left-0 w-full mt-4 transition-all duration-500", isFocused ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-2 pointer-events-none")}>
